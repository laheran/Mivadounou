...
public class CommandeDialogFragment extends DialogFragment
...
//                Toast.makeText(getActivity(), "From dialog Fragment", Toast.LENGTH_SHORT).show();
    ...
    public void sendCommande() {
        ...
        String newCommandeKey = mDatabase.child(FirebaseRef.USER_COMMANDES).push().getKey();
        
        String newRestauCommandenotificationKey = mDatabase.child(FirebaseRef.RESTAU_COMMANDE_NOTIFICATIONS)
                .child(commandeMenu.getRestauKey()).push().getKey();
        
        NotificationMessage notificationMessage = new NotificationMessage(
                "Nouvelle commande reçu\n" +
                        commandeMenu.getMenuTitle() + " Quantité : " + commandeMenu.getQuantity() + " Prix unitaire : " + commandeMenu.getUnitPrice() + " Prix totale : " + commandeMenu.getTotalPrice() + " Pour le " + dateFormat.format(calendar.getTime()) + "Par : " + commandeMenu.getUserName(), false);
        
        Map<String, Object> commandeMenuValues = commandeMenu.toMap();
        
        Map<String, Object> restauCommandenotificationValue = notificationMessage.toMap();
        
        Map<String, Object> childUpdates = new HashMap<>();
        childUpdates.put("/" + FirebaseRef.COMMANDE + "/" + newCommandeKey, commandeMenuValues);
        childUpdates.put("/" + FirebaseRef.USER_COMMANDES + "/" + commandeMenu.getUserKey() + "/" + newCommandeKey, commandeMenuValues);
        childUpdates.put("/" + FirebaseRef.RESTAU_COMMANDES + "/" + commandeMenu.getRestauKey() + "/" + newCommandeKey, commandeMenuValues);
        childUpdates.put("/" + FirebaseRef.RESTAU_COMMANDE_NOTIFICATIONS + "/" + commandeMenu.getRestauKey() + "/" + newRestauCommandenotificationKey, restauCommandenotificationValue);
